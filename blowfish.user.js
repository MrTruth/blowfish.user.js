...
// @description This UserScript for encryption / decryption of text on a Web page. Usage: encode: select text, press ctrl+Q, enter password, press ok; decode: select text, press ctrl+shift+Q, enter password, press ok
// @author github.com/MrTruth
// @version 1.0.3
// ==/UserScript==
...
if(document.getSelection() != ''){
  ...
  str = str.toString();
  crstring = str;
  lenghtstr = 8-(crstring.length - Math.floor((crstring.length/8))*8);
  if(lenghtstr != 8){
  	for (var i = 0; i < lenghtstr; ++i) {
        crstring = crstring+' ';
    }
	}
  }
...
function decryption()
{
	crstring = unescape(crstring);
	encrypt = crstring.replace(/ /g,"");
	var bf = new Blowfish(cryptPasswd);
	var plaintext = unescape(bf.decrypt(encrypt));
	printText(plaintext);
}
...
{
	var bf = new Blowfish(cryptPasswd);
	var ciphertext = bf.encrypt(escape(crstring));
	var encrypted = '';
	for (var i = 0; i < ciphertext.length; i=i+16) {
        encrypted = encrypted+' '+ciphertext.substring(i, i+16);
    }
...
function printText(textString)
{
	...
	e.innerHTML = 'var codewindow = window.open(\'\',\'new\',\'width=600,height=400,toolbar=1\'); textString = \''+escape(textString)+'\'; var ediv = codewindow.document.createElement("textarea"); ediv.style.width = \'100%\'; ediv.style.height = \'100%\'; ediv.style.font = \'10pt Courier New, monospace\'; ediv.innerHTML = unescape(textString); codewindow.document.getElementsByTagName("body")[0].appendChild(ediv);';
...
function keypressed()
{
	...
	{
		cryptPasswd = prompt('Password:',''); encryption();
	}
	
	if(event.shiftKey && event.ctrlKey && event.keyCode == 81)
	{
		cryptPasswd = prompt('Password:',''); decryption();
	}
}
if (window.Event)
{
